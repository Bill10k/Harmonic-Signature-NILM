function sim = model_four_appliances(params)


%MODEL_FOUR_APPLIANCES
% Clean appliance current simulation for NILM harmonic-signature project.
requiredFields = {'fs','f0','duration','windowLength','hopLength'};

for i = 1:numel(requiredFields)
    if ~isfield(params, requiredFields{i})
        error('Missing parameter: %s', requiredFields{i});
    end
end
fs = params.fs;
f0 = params.f0;
duration = params.duration;
windowLengthSec = params.windowLength;
hopLengthSec = params.hopLength;

if isfield(params,'plotResults')
    plotResults = params.plotResults;
else
    plotResults = true;
end
% Output:
...
outDir = fullfile('figures','signal_generation');
if ~exist(outDir, 'dir')
    mkdir(outDir);
end
